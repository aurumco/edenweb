...
import { characterApi, runApi, Character as ApiCharacter, Run as ApiRun, CharacterSpec } from "@/lib/api";
...
function PlayerDashboardContent() {
  ...
  async function onSubmit(data: CharacterForm) {
    ...
    const specs: CharacterSpec[] = roles.map((r) => ({
      spec: r, // Using Role as Spec name as per user simplified input flow
      role: r.toUpperCase() as "TANK" | "HEALER" | "DPS",
      type: r === "DPS" ? "Melee" : "Ranged" // Defaulting for now, can be refined
    }));
                        ...
                        <TableRow key={c.id}>
                          ...
                          <TableCell>
                            <div className="flex flex-wrap items-center gap-1.5">
                              {(c.specs || []).map((specItem, idx) => {
                                // specItem is now CharacterSpec object
                                const spec = specItem.spec || "Unknown";
                                let badgeClass = "text-xs rounded-full px-2 py-0.5";
                                if (specItem.role === "TANK") badgeClass += " bg-lime-500/50 text-lime-700 dark:bg-lime-500/10 dark:text-lime-400";
                                else if (specItem.role === "HEALER") badgeClass += " bg-fuchsia-500/50 text-fuchsia-700 dark:bg-fuchsia-400/10 dark:text-fuchsia-400";
                                else badgeClass += " bg-rose-500/50 text-rose-700 dark:bg-rose-400/10 dark:text-rose-400";
                                return (
                                  <Badge key={`${spec}-${idx}`} variant="outline" className={badgeClass}>{spec}</Badge>
                                );
                              })}
                            </div>
                          </TableCell>
